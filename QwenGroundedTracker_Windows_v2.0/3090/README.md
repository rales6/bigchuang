...
这个目录只放 RTX 3090 Linux 端代码。3090 负责 Qwen、YOLO、CSRT 跟踪、边界提取和安全仲裁；Windows 端只负责摄像头采集、网页显示、聊天记录和把画面通过 WebSocket 发过来。
...
Windows 网页端使用本地 SSH 隧道访问 3090。Windows 端常用启动命令是：
...
如果 Windows 端发现 `127.0.0.1:8001` 不通，会自动尝试启动：
...
python -m pip install -r requirements/base.txt -r requirements/qwen.txt -r requirements/yolo.txt
```

如果 CSRT 不可用，重新安装 OpenCV contrib 版本：

```bash
python -m pip uninstall -y opencv-python opencv-python-headless opencv-contrib-python opencv-contrib-python-headless
python -m pip install "opencv-contrib-python-headless==4.10.0.84"
python -c "import cv2; print(hasattr(cv2, 'TrackerCSRT_create'), hasattr(cv2, 'legacy') and hasattr(cv2.legacy, 'TrackerCSRT_create'))"
```

只要上面最后一行至少有一个 `True`，CSRT 跟踪器就可用。
