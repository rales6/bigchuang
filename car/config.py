...
# 树莓派控制链路模式。实车只启用 BLE，避免 USB/UART 与 BLE 同时复用协议端点。
# UART 参数保留用于离线诊断，但 PI_LINK_MODE="ble" 时不会创建该 UART。
PI_LINK_MODE = "ble"
PI_UART_ID = 0
...
# BLE 使用 Nordic UART Service (NUS)，传输 Vehicle Link V2 字节帧。
PI_BLE_ENABLED = True
...
FIRMWARE_BUILD = "2026.08.02-ble-v17-stable-link"
