...
至少应覆盖车体中心到最外缘的距离，并留出定位误差。运动命令默认有 700 ms
有效期，并以 450 ms 间隔发送完全相同的非阻塞保活帧；首次下发、变速和变向仍
必须收到 ACK。雷达停止出帧、程序卡住或连续保活丢失后，ESP32 TTL 会停车。
...
栅格会保留原世界坐标并自动向相应方向扩展。矩形房间方向校正默认开启，用雷达
自动学习两组正交墙轴并缓慢修正累计航向漂移；非正交场地可使用
`--no-manhattan` 显式关闭。
...
堵转也无法继续加力，并且 450 mm/s 的倒退脱困指令会被错误压到 300 mm/s。直行
过程中若 BLE 中断，第一次重连后的起步补偿会直接提高到 +120 mm/s；连续再次中断
提高到 +160 mm/s，但最终速度仍不超过 `--max-drive-speed`。实际运动过快或匹配
质量下降时补偿会自动回退。
