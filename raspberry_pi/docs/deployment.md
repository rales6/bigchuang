...
程序无论使用哪条链路，都保持 250 ms 心跳/速度刷新。ESP32 超过 800 ms 没收到
有效帧会立即停止底盘和机械臂；链路切换期间这项保护仍然有效。

BLE 运动帧使用 NUS 的 write-without-response，再由 Vehicle Link V2 的 CRC、ACK、
序号幂等重试保证可靠性，避免每个20字节分片都进行一次ATT往返。ESP32默认关闭
主动状态广播，状态只在树莓派查询时返回，防止状态通知与运动ACK争用通知队列。
树莓派会缓存首次扫描到的BLE地址，短暂失败后直接重连，并将单次GATT操作限制在
1.5秒内；设备扫描超时不再被错误地用于每次写操作。
