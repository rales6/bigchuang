...
`Esp32Client.set_twist()`。工作流程是：BLE 无动作心跳、初始原地扫描、寻找可达
前沿、低速前进、每 8 个有效扫描重新规划。定位始终使用完整 360° 当前帧、完整
上一帧、最近 8 帧局部子图和长期关键帧；只有写占据栅格与避障时才裁成前方 160°。
前沿目标默认取所有可达白色/未知边界中直线距离最远的点，失败目标附近会暂时排除，
但不会连带隐藏同一边界上的其他可达点。
...
网页不读取仿真器真值，也不会参与底盘控制。地图默认以 2 Hz 在内存中编码，不会每帧
写入磁盘；状态指标仍随每个扫描帧更新。可通过
...
更远区域保持未探索。导出地图时，小于 `--wall-gap-max` 的水平、垂直和斜向黑线
缺口会被拼接。弱单次回波和小障碍点会先删除，再进行墙线拼接，因此不会把散点
合成为假墙；该清理只影响图像，不篡改定位与规划使用的概率栅格。

占据地图也不是只增不减，但修改采用保守证据窗口：连续十五个不同雷达帧的自由
射线与旧墙冲突才会撤销旧墙，同一帧中的多条射线只计一次；同一位置和朝向至少
保留八帧独立证据，新扫描每次最多淘汰一帧最旧
证据。15 分钟运行会保留最多 1800 帧，不再因原来的 320 帧上限而把早期房间整片
删掉。周期快照不阻塞车辆，最终停车保存时会把保留的关键帧重放到全新网格；
...
栅格会保留原世界坐标并自动向相应方向扩展。

导出图像时，短缺口拼接后的黑色边界若完全包围灰色未知区域，内部未知像素会填成
黑色。已经被雷达确认可通行的白色区域不会因外围墙闭合而被涂黑，因此房间内部
仍可用于规划。
...
程序也会默认每 50 个有效扫描覆盖保存一次，避免意外断电丢失全部结果。
